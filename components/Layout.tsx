...
const Layout: React.FC<LayoutProps> = ({ children, onOpenCreateProfile }) => {
  ...
  return (
    <div className={`flex h-screen w-screen overflow-hidden transition-all duration-500 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} flex-col lg:flex-row`}>
      ...
      {/* LEFT SIDEBAR (FOLDERS) - Persistent on Desktop */}
      <div className={`${activeTab === 'folders' ? 'fixed inset-0 z-50 flex' : 'hidden'} lg:relative lg:flex lg:h-full lg:z-20 border-r ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
      ...
      <main className={`flex-1 flex flex-col h-full overflow-hidden z-10 relative ${activeTab === 'folders' ? 'hidden lg:flex' : 'flex'}`}>
        ...
        {/* COMPACT HEADER - Enhanced for PC */}
        <header className={`px-4 lg:px-10 py-3 lg:py-6 flex items-center justify-between sticky top-0 z-40 transition-all ${isDark ? 'bg-slate-900/80' : 'bg-white/90'} backdrop-blur-xl border-b ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          <div className="flex items-center gap-3 lg:gap-6 flex-1 min-w-0">
            <h1 className="text-lg lg:text-3xl font-black tracking-tight truncate flex items-center gap-3 shrink-0">
              <span className="truncate">{activeTab === 'leaderboard' ? 'Rankings' : currentFolderName}</span>
              <div className="shrink-0 relative flex h-2 w-2 lg:h-3 lg:w-3" title={`Socket: ${socketStatus}`}>
                ...
                <span className={`relative inline-flex rounded-full h-2 w-2 lg:h-3 lg:w-3 ${socketStatus === 'connected' ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
              </div>
            </h1>
            ...
            {activeParty?.is_parking_enabled && (
              <div className="hidden sm:flex relative" ref={parkingRef}>
                ...
                >
                  ...
                  <span className="hidden xl:inline">Available Slots</span>
                  <span className="xl:hidden">Slots</span>
                </button>
                
                {isParkingOpen && (
                  <div className="absolute top-16 left-0 w-72 glass-card p-6 rounded-[2rem] border border-white/10 shadow-2xl animate-in slide-in-from-top-4 duration-300 z-50 overflow-hidden">
                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-4 border-b border-white/5 pb-2">Hub Capacity</p>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-lg font-black text-white uppercase tracking-tighter">{freeSpots} Vacant</span>
                      <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse ring-4 ring-emerald-500/10"></div>
                    </div>
                    <div className="max-h-56 overflow-y-auto custom-scrollbar space-y-1.5 mb-5 pr-1">
                       {Array.from({ length: Math.min(freeSpots, 12) }).map((_, i) => (
                         <div key={i} className="bg-white/5 p-3 rounded-xl text-[9px] font-black text-slate-400 flex justify-between group hover:bg-indigo-500/10 transition-colors">
                            <span className="uppercase">Node ID-{1000 + i}</span>
                            <span className="text-emerald-400 uppercase tracking-widest">Available</span>
                         </div>
                       ))}
                    </div>
                    <button className="w-full bg-indigo-600 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] text-white shadow-xl hover:bg-indigo-700 transition-all active:scale-[0.97]">Establish Node</button>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex items-center gap-3 lg:gap-4 shrink-0">
            {/* FORCE REFRESH BUTTON (Visible on Mobile only, integrated to Right Nav for Desktop) */}
            <button 
              onClick={handleManualRefresh} 
              className={`lg:hidden p-2.5 rounded-xl transition-all relative border-2 ${
                isDark ? 'bg-slate-800 hover:bg-slate-700' : 'bg-slate-100 hover:bg-slate-200 shadow-sm'
              } ${
                networkQuality === 'good' ? 'border-emerald-500/50 text-emerald-500' : 'border-red-500/50 text-red-500'
              }`}
            >
            ...
            {isDev && (
              <button onClick={() => setIsWorkflowMode(!isWorkflowMode)} className={`p-2.5 lg:p-3.5 rounded-2xl transition-all border-2 ${isWorkflowMode ? 'bg-emerald-500 border-emerald-400 text-white shadow-lg' : 'bg-slate-800 border-slate-700 text-slate-400'}`} title="Design Canvas">
            ...
            <button onClick={() => setIsNotifOpen(true)} className={`p-2.5 lg:p-3.5 rounded-2xl transition-all relative ${isNotifOpen ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
              {unreadNotifications > 0 && <span className="absolute -top-1 -right-1 flex h-5 w-5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span><span className="relative inline-flex rounded-full h-5 w-5 bg-red-500 border-2 border-white dark:border-slate-900 text-[10px] font-black items-center justify-center text-white">{unreadNotifications}</span></span>}
            </button>

            <button 
              onClick={onOpenCreateProfile} 
              disabled={isCreationDisabled || !selectedFolderId} 
              className={`${isCreationDisabled || !selectedFolderId ? 'bg-slate-200 dark:bg-slate-800 text-slate-400' : 'bg-indigo-600 text-white shadow-xl shadow-indigo-500/20'} px-5 lg:px-8 py-2.5 lg:py-4 rounded-2xl text-[10px] lg:text-xs font-black transition-all flex items-center gap-3 shrink-0 active:scale-[0.95]`}
            >
              <svg className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d={isCreationDisabled ? "M5 13l4 4L19 7" : "M12 4v16m8-8H4"} /></svg>
              <span className="uppercase tracking-[0.2em]">{!selectedFolderId ? 'Select' : isCreationDisabled ? 'Joined' : 'Join'}</span>
            </button>
          </div>
        </header>

        {/* MAIN SCROLLABLE AREA - Optimized for PC Feasibility */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:p-12 pb-32 lg:pb-12 bg-slate-50 dark:bg-slate-950/20">
        ...
        {/* Scroll Top Button */}
        <button onClick={scrollToTop} className={`fixed bottom-28 lg:bottom-12 right-24 lg:right-32 p-4 rounded-full shadow-2xl transition-all duration-500 z-[90] border-2 group ${showScrollTop ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-10 scale-50 pointer-events-none'} ${isDev ? 'bg-emerald-500 border-emerald-400 text-white shadow-emerald-500/20' : 'bg-indigo-600 border-indigo-500 text-white shadow-indigo-500/20'}`} aria-label="Scroll to top"><svg className="w-6 h-6 transform group-hover:-translate-y-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 10l7-7m0 0l7 7m-7-7v18" /></svg></button>
      </main>

      {/* DESKTOP RIGHT NAVIGATION BAR - Ensures Parity for PC Users */}
      <aside className={`hidden lg:flex flex-col w-20 xl:w-24 h-full border-l transition-colors duration-500 items-center py-8 gap-10 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
        <div className="flex flex-col items-center gap-8">
          <button 
            onClick={() => setActiveTab('community')}
            className={`flex flex-col items-center gap-2 transition-all p-3 rounded-2xl group ${activeTab === 'community' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
            title="Hub Feed"
          >
            ...
            <span className="text-[7px] xl:text-[8px] font-black uppercase tracking-[0.15em]">Feed</span>
          </button>

          <button 
            onClick={() => setActiveTab('leaderboard')}
            className={`flex flex-col items-center gap-2 transition-all p-3 rounded-2xl group ${activeTab === 'leaderboard' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
            title="Leaderboard"
          >
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
            <span className="text-[7px] xl:text-[8px] font-black uppercase tracking-[0.15em]">Rank</span>
          </button>
        </div>

        <div className={`w-10 h-px ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`} />

        <div className="flex flex-col items-center gap-8 mt-auto">
          <button 
            onClick={handleManualRefresh}
            className={`flex flex-col items-center gap-2 transition-all p-3 rounded-2xl group ${
              networkQuality === 'good' ? 'text-emerald-500 hover:bg-emerald-500/10' : 'text-red-500 hover:bg-red-500/10'
            }`}
            title={`Force Sync (Matrix Quality: ${networkQuality})`}
          >
            <svg className={`w-7 h-7 ${isRefreshing ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
            <span className="text-[7px] xl:text-[8px] font-black uppercase tracking-[0.15em]">Sync</span>
          </button>

          <button 
            onClick={() => setTheme(isDark ? 'light' : 'dark')}
            className={`flex flex-col items-center gap-2 transition-all p-3 rounded-2xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800`}
            title="Toggle Visual Mode"
          >
            {isDark ? <svg className="w-7 h-7 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707M16.071 16.071l.707.707M7.929 7.929l.707-.707M12 8a4 4 0 110 8 4 4 0 010-8z" /></svg> : <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>}
            <span className="text-[7px] xl:text-[8px] font-black uppercase tracking-[0.15em]">{isDark ? 'Light' : 'Dark'}</span>
          </button>
        </div>
      </aside>
