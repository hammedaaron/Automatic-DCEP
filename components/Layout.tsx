...
const Layout: React.FC<LayoutProps & { onOpenCreateProfile: () => void }> = ({ children, onOpenCreateProfile }) => {
  ...
  useEffect(() => {
    const updateStatus = () => {
      ...
      if (!isOnline) { setNetworkQuality('poor'); return; }
      if (connection) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (parkingRef.current && !parkingRef.current.contains(event.target as Node)) { setIsParkingOpen(false); }
    };
  ...
  return (
    <div className={`flex h-screen w-screen overflow-hidden transition-all duration-500 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} flex-col lg:flex-row`}>
      ...
      <main className={`flex-1 flex flex-col h-full overflow-hidden z-10 relative ${activeTab === 'folders' ? 'hidden lg:flex' : 'flex'}`}>
        ...
        {/* ENHANCED TOP NAVIGATION BAR FOR PC/MOBILE */}
        <header className={`px-4 lg:px-8 py-3 lg:py-4 flex items-center justify-between sticky top-0 z-40 transition-all ${isDark ? 'bg-slate-900/80' : 'bg-white/95 shadow-sm'} backdrop-blur-xl border-b ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
          <div className="flex items-center gap-4 lg:gap-8 flex-1 min-w-0">
            ...
            <nav className="hidden lg:flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl border border-slate-200 dark:border-slate-700">
              <button 
                ...
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'community' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-indigo-500'}`}
              >
                Feed
              </button>
              <button 
                onClick={() => setActiveTab('leaderboard')}
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'leaderboard' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-indigo-500'}`}
              >
                Rankings
              </button>
            </nav>
          ...
          <div className="flex items-center gap-2 lg:gap-3 shrink-0">
            {/* PARKING (Integrated into global header) */}
            {activeParty?.is_parking_enabled && (
              <div className="relative" ref={parkingRef}>
                <button 
                  onClick={() => setIsParkingOpen(!isParkingOpen)}
                  className={`flex items-center gap-2 px-3 lg:px-4 py-2 lg:py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                    isParkingOpen ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-400 hover:border-indigo-500'
                  }`}
                >
                ...
                {isParkingOpen && (
                  <div className="absolute top-12 right-0 w-64 glass-card p-5 rounded-[2rem] border border-white/10 shadow-2xl animate-in slide-in-from-top-4 duration-300 z-50">
                    <p className="text-[9px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-3">Capacity</p>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-black text-white uppercase">{freeSpots} Vacant</span>
                      <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
                    </div>
                    <button className="w-full bg-indigo-600 py-3 rounded-xl text-[9px] font-black uppercase text-white shadow-xl hover:bg-indigo-700 transition-all">Establish Node</button>
                  </div>
                )}
              </div>
            )}
            ...
            <button 
              ...
              className={`p-2 lg:p-2.5 rounded-xl transition-all relative border-2 ${
            ...
            {/* THEME (Visible on PC Top Bar) */}
            <button 
              onClick={() => setTheme(isDark ? 'light' : 'dark')}
              className={`hidden lg:flex p-2.5 rounded-xl transition-all ${isDark ? 'bg-slate-800 text-amber-400' : 'bg-slate-100 text-slate-500'}`}
            >
            ...
            <button 
              ...
              className={`${isCreationDisabled || !selectedFolderId ? 'bg-slate-200 dark:bg-slate-800 text-slate-400' : 'bg-indigo-600 text-white shadow-lg'} px-4 lg:px-6 py-2 lg:py-2.5 rounded-xl text-[10px] font-black transition-all flex items-center gap-2 active:scale-[0.95]`}
            >
        ...
        <nav className={`lg:hidden fixed bottom-0 left-0 right-0 z-[100] px-4 pb-8 pt-3 ${isDark ? 'bg-slate-950/95' : 'bg-white/95'} backdrop-blur-2xl border-t ${isDark ? 'border-slate-800' : 'border-slate-200'} flex items-center justify-around shadow-[0_-10px_40px_rgba(0,0,0,0.3)]`}>
          <button onClick={() => setActiveTab('folders')} className={`flex flex-col items-center gap-1.5 p-2 transition-all ${activeTab === 'folders' ? 'text-indigo-500 scale-110' : 'text-slate-500'}`}>
          ...
          <button onClick={() => setActiveTab('community')} className={`flex flex-col items-center gap-1.5 p-2 transition-all ${activeTab === 'community' ? 'text-indigo-500 scale-110' : 'text-slate-500'}`}>
          ...
          <button onClick={() => setActiveTab('leaderboard')} className={`flex flex-col items-center gap-1.5 p-2 transition-all ${activeTab === 'leaderboard' ? 'text-indigo-500 scale-110' : 'text-slate-500'}`}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
            <span className="text-[8px] font-black uppercase">Rank</span>
          </button>
          <button onClick={() => setTheme(isDark ? 'light' : 'dark')} className={`flex flex-col items-center gap-1.5 p-2 transition-all text-slate-500`}>
            ...
            <span className="text-[8px] font-black uppercase">{isDark ? 'Light' : 'Dark'}</span>
          </button>
        </nav>
